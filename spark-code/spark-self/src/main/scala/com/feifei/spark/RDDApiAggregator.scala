...
//    用相同的key的sum和count值相除（用join的技术让相同key的以一行的形式呈现出来）
    ...
    //通过combine一步把，sum,count,max,min,avg所有数据一步全部拿到
    data.combineByKey(
      (v) => (v, 1, v, v),
    ...
    ).mapValues(t4=>(t4._1,t4._2,t4._3,t4._4,t4._1/t4._2)).foreach(println)

    //任何进行聚合运算的场景都可以优化成combineByKey的方式来降低shuffle的次数
    //这就要充分了解combineByKey的三个函数的利用了
    while (true) {}
  }
